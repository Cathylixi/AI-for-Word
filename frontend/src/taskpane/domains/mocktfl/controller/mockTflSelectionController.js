import { getMockTflExampleEntries } from "../api/references";
import { autoSelectMockTflSections } from "../api/autoSelect";
...
function setMockTflWorkItemOptions(selectEl, entries) {
  ...
  items
    ...
    .sort((a, b) => {
      ...
      const bo = typeof b.order === "number" ? b.order : Number.MAX_SAFE_INTEGER;
      if (ao !== bo) return ao - bo;
      const at = String(a.type || "").trim().toUpperCase();
      const bt = String(b.type || "").trim().toUpperCase();
      if (at !== bt) return at.localeCompare(bt, "en");
      return String(a.number || "").localeCompare(String(b.number || ""), "en");
    })
    .forEach((e) => {
    const type = String(e.type || "").trim().toUpperCase();
    const number = String(e.number || "").trim();
